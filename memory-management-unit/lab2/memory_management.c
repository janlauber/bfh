...
/*
 * Lock for the memory management
 */
pthread_mutex_t mutex;

int memory_free_data(void) {
  // Free all memory in tlb
  for (int i = 0; i < g_tlb_size; i++) {
	tlb[i].page_number = -1;
	tlb[i].frame_number = -1;
	tlb[i].last_access = -1;
  }
  // Free all memory in page table
  for (int i = 0; i < g_free_frames; i++) {
	page_table[i] = -1;
  }
  // Free memory in memory
  free(memory);
  free(tlb);
  free(page_table);
  return 0;
...
int get_physical_address(uint64_t virtual_address,
			 ...
			 int* tlb_hit)
{
	// Lock the memory management
	pthread_mutex_lock(&mutex);

	// Flush TLB if necessary
	if (tlb[process_id].last_access < tlb[process_id].last_access) {
		tlb[process_id].page_number = -1;
		tlb[process_id].frame_number = -1;
		tlb[process_id].last_access = -1;
	}
